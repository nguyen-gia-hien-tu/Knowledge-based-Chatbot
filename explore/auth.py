...
Here is an example of a very simple secrets.toml file for this project:
FIREBASE_API_KEY="..."
COOKIE_KEY="some_cookie_name_here"
[firebase_auth_token]
...
success, error = partial(st.success, icon="✅"), partial(st.error, icon="🚨")
...
def forgot_password_form(preauthorized: Union[str, Sequence[str], None]) -> None:
    ...
    response = post_request(url, json=payload)
    if response.status_code == 200:
        success(f"Password reset link has been sent to {email}")
        return
    return
...
def register_user_form(preauthorized: Union[str, Sequence[str], None]) -> None:
    ...
    if strength < 50:
        ...
        st.warning("Password is too weak. Please choose a stronger password.", icon="⚠️")
        return
    auth.create_user(
    ...
    response = post_request(url, json=payload)
    if response.status_code != 200:
        error(f"Error sending verification email: {parse_error_message(response)}")
        return
    success(
...
def update_display_name_form(
    ...
    success("Successfully updated user display name.")
    return


def token_encode(exp_date: dt.datetime) -> str:
...
def cookie_is_valid(cookie_manager: stx.CookieManager, cookie_name: str) -> bool:
    ...
    Notes
    -----
    This function checks if the specified reauthentication cookie is present in the cookies stored
    by the cookie manager, and if it is valid. If the cookie is valid, this function updates the
    session state of the Streamlit app and authenticates the user.
    """
    ...
    if (
        ...
        and token["exp_date"] > dt.datetime.now(dt.UTC).timestamp()
...
def login_form(
    ...
    try:
        ...
        if not user.email_verified:
            error("Please verify your e-mail address.")
            return
...
def main() -> None:
    ...
    if not cookie_is_valid(cookie_manager, cookie_name) and not_logged_in(
        cookie_manager, cookie_name, preauthorized=("gmail.com", "email.com")
    ):
        return
